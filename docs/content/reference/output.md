...
describes what users get. Pick a format with `-o`, or let cgvvn choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cgvvn <command> -o table   # aligned columns for reading
cgvvn <command> -o jsonl   # one JSON object per line, for piping
cgvvn <command> -o json    # a single JSON array
cgvvn <command> -o csv     # spreadsheet friendly
cgvvn <command> -o tsv     # tab-separated
cgvvn <command> -o url     # just the URL column
cgvvn <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cgvvn <command> --fields id,title,url
```
...
```bash
cgvvn <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cgvvn <command>            # a table, because this is a terminal
cgvvn <command> | wc -l    # JSONL, because this is a pipe
```
